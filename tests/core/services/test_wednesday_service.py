...
from core.models.social_post import TwitterPostModel, TwitterReplyModel, SocialPostModel, SocialReply
...
@pytest.mark.parametrize("start_time_iso,expected_message", [
    (LATEST_WEDNESDAY_MIDNIGHT + datetime.timedelta(days=1), "Still 4 days and 22 hours until Earth sees Wednesday."),
    (EARLIEST_WEDNESDAY_MIDNIGHT - datetime.timedelta(days=1, hours=2), "Still 1 day and 2 hours until Earth sees Wednesday."),
    ...
    (EARLIEST_WEDNESDAY_MIDNIGHT - datetime.timedelta(minutes=1, seconds=59), "Earth is just 1 minute and 59 seconds from Wednesday."),
    (EARLIEST_WEDNESDAY_MIDNIGHT - datetime.timedelta(seconds=1), "Buckle up! 1 second until Earth enters Wednesday."),
])
@freeze_time(WEDNESDAY_MIDNIGHT_UTC)
def test_build_twitter_reply_not_wednesday_message_format(start_time_iso: datetime.datetime, expected_message):
    """Tests if the date calculation and message work, compares the earliest wednesday TZ and the latest wednesday TZ to see if it's wednesday somewhere on earth"""
    with (freeze_time(start_time_iso)):
        ...
        assert mock_twitter_client.find_recent_hashtags_posts.call_args.kwargs["hashtags_to_search"] == constants.SEARCH_TERMS_WEDNESDAY_HASHTAGS
...
def test_when_is_wednesday_then_reply_to_recent_wednesday_tweets_replies_with_wednesday_message():
    mock_twitter_client.find_recent_hashtags_posts.return_value = [
        TwitterPostModel(id="2", message="is it wednesday?")
    ]
